...
public class HttpAsyncLoader extends AsyncTaskLoader<NewsDictionary> {
    ...
    public HttpAsyncLoader(@NonNull Context context, String string) {
        super(context);
        mContext = context;
        Calendar cal = Calendar.getInstance();
        //1か月前から取得するので、MONTHは+1しない
        mDay = cal.get(Calendar.YEAR) + "-" + cal.get(Calendar.MONTH) + "-" + cal.get(Calendar.DAY_OF_MONTH);
        if(string != null) {
            searchWord = string;
        } else {
            searchWord = mContext.getString(R.string.default_news);
        }
        mReqUrl = "http://newsapi.org/v2/everything?q=" + searchWord + "&from=" + mDay + "&sortBy=publishedAt&apiKey=9f1a1981a78941b6848b054c6d0cd651";
    }
